...
- Specialists can read and write anywhere available to the current OS user. The parent provides known source paths, may acquire any source directly, owns acquisition and worktrees for target repositories, and directs investigative writes to the shared sandbox. Among specialists, only Librarian may acquire a missing external-evidence source under its role contract, and every Librarian network or service operation must remain read-only. Oracle review may create a disposable target copy in the shared sandbox only when its protocol requires one.
- Do not decide a question delegated to a running specialist. Do not poll detached consultations.
- Use Oracle for consequential reasoning and completed-diff review, Finder for retrieval inside the task's target repositories, Librarian for external evidence across upstream repositories, GitHub, published documentation, standards, registries, and prior art, Reporter for goal-directed extraction across supplied transcript sources, and the parent for implementation.

## Activate or rehydrate

Setup needs a verified 16-digit Garcon chat ID; the user need not supply it. On activation or rehydration:

1. Unless handling the activation's disclosure, emit exactly `<garcon-get-chat-id />` as the entire assistant text message—never in reasoning or with surrounding whitespace, prose, or formatting—then end the turn. Emit at most once across its originating run and any steering, rehydration, or direct control continuation.
2. Do not call a tool, delay, or poll. Garcon first steers the emitting run; definitive non-delivery starts one direct control run. Both routes belong to this activation. If neither resumes the activation, setup does not run; the visible `Chat ID auto-discovery` notice reports why.
3. Accept only post-marker input equal to `<garcon-chat-id>[0-9]{16}</garcon-chat-id>` after removing surrounding whitespace and the provider's optional steering preamble. Never derive IDs from host state, tools, files, searches, sandbox paths, or specialist output. Exception: this skill's intact packet from a successful setup call.
4. The fresh disclosure for this activation is authoritative. If work resumes without one, use an explicit current-chat user ID, then the trusted packet; if both exist and disagree, stop and ask. Pass the selected ID to setup. If neither is available, ask for the 16-digit ID in the current Garcon URL (`/chat/<id>`) or a prior disclosure notice.

`--garcon-path` is optional. Setup autodetects a valid Garcon repository on first run and preserves the recorded root on reruns. If setup cannot resolve one, stop and ask the user for the Garcon path. On first setup, run from this skill directory:
...
Oracle supports user-directed per-run `--spec` and `--additional-spec` overrides. Use them only when the user supplied each exact spec, and never persist runtime specs into configuration. The runtime packet defines their syntax, concurrency, attribution, and result handling.
